...
public class explodingSootingZombieII : MonoBehaviour
{
    ...
    void Update()
    {
        if (alive && !charrecter.GetComponent<hp>().ispaused)
        {
            ...
            {
                speed = 3;
                if (!Physics2D.Linecast(transform.position, groundcheckR.position, 1 << LayerMask.NameToLayer("Ground")) && !Physics2D.Linecast(transform.position, groundcheckR.position, 1 << LayerMask.NameToLayer("flyingGround"))||wallR)
                {
                    if (tmer)
                    {
                        tmer = false;
    ...
    private IEnumerator atackEnd()
    {
        ...
        if ((transform.position.x - charrecter.transform.position.x)>0)
        {
            pula1.GetComponent<explodingBall>().speedx = 0.8f*(transform.position.x - charrecter.transform.position.x);
        ...
        {
            pula1.GetComponent<explodingBall>().speedy = (charrecter.transform.position.y - transform.position.y)*2 + pula1.GetComponent<explodingBall>().rot * (transform.position.x - charrecter.transform.position.x)*-0.3f;
        }
        else
        {
            pula1.GetComponent<explodingBall>().speedy = pula1.GetComponent<explodingBall>().rot * (transform.position.x - charrecter.transform.position.x)*-0.3f;
        }
